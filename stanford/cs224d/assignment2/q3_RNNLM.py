...
class RNNLM_Model(LanguageModel):
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('Projection'):
      ...
      outputs = [tf.matmul(output, U) + b_2 for output in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as scope:
      ...
      for input in inputs:
        current_state = tf.nn.sigmoid(
          tf.matmul(current_state, H) + tf.matmul(input, I) + b_1
        )
        rnn_outputs.append(current_state)
...
def test_RNNLM():
  ...
  with tf.Session() as session:
    ...
    tf.train.write_graph(graph_def, '.', 'tf-model_graph')
