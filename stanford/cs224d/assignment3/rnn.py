...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            curr_node_tensor = tf.expdand_dims(tf.gather(embedding, word_id), 0)
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            node_concat = tf.concat(1, [node_tensors[node.left], node_tensors[node.right]])
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope("Projection", reuse=True):
            U = tf.get_variable("U")
        l2_loss = 0.5 * self.config.l2 * (tf.nn.l2_loss(W1) + tf.nn.l2_loss(U))
        error = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, labels))
