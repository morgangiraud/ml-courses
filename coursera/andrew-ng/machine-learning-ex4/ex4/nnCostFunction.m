...
X = [ones(m, 1), X];
z_2 = Theta1 * X';
a_layers_2 = sigmoid(z_2);
a_layers_2 = [ones(1, m); a_layers_2];
z_3 = Theta2 * a_layers_2;
h_theta = sigmoid(z_3); % size(h_theta) 10 x m
...
cost = sum(sum(- y_vec .* log(h_theta) - (1 - y_vec) .* log(1 - h_theta))); % size(result) m x m
